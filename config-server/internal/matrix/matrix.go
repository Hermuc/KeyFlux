...
func DigitalRain(hasError <-chan struct{}, rainDone chan<- struct{}) {
	...
	if err != nil {
		return // 无终端时直接退出, defer close(rainDone) 保证调用方不阻塞
	}
...
outer:
	for {
		...
		banner := []string{
			" ------------------------------------------------------------------ ",
			"                                                                    ",
			"               KeyFlux config server is running...                 ",
			"                                                                    ",
			" ------------------------------------------------------------------ ",
		}
...
// randN 返回 [0, n) 的随机数, n<=0 时返回 0, 避免终端窗口过小时 rand.Intn 负数 panic
