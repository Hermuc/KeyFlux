...
- ⚡ **Selected action system** — select text or files, then press a hotkey to trigger a preset action; rules are editable visually. Two match types: file extensions (with customizable group quick-fill) and text features (URL / path / magnet link / Bilibili video ID (AV / BV) / plain text), strictly paired with dedicated actions
