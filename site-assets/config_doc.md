...
点开「选项」页会发现有的触发键以星号开头 ( 例如 3 模式的 `*3` )，而有的没有星号 ( 例如 `Tab` )
...
新增自定义热键时要填热键写法，规则和「 💎 输入文本或按键 」里讲的组合键一致，这里只列常用的，避免重复可回看前面的「 ➤ 第一行 」
...
- `^` 表示 Ctrl 键
- `+` 表示 Shift 键
- 修饰符可以叠加，比如 `^!c` 表示 Ctrl + Alt + C，`+#enter` 表示 Shift + Win + Enter

### 常用按键名

热键里的按键名直接写即可 ( 和输入按键不同，不用大括号 `{}` 包裹 )，常用的有:
...
- `rbutton` 鼠标右键
- `F1` 到 `F24` 功能键，例如 `F5`
- `a` 到 `z` 字母，`0` 到 `9` 数字，例如 `k`、`3`

更多特殊按键可参考 [AHK 按键列表](https://wyagd001.github.io/v2/docs/KeyList.htm#keyboard)
