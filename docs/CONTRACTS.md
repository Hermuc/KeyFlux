...
| 2026-09-23 | **快速切换移除 Ctrl+G 绑定, 触发语义收敛为「文件对话框出现即自动」** (用户要求移除现有快捷键且不绑定新快捷键, 自行设计非快捷键自动触发方案)。核实: Ctrl+G 真源是 `data/config.json` keymaps[12] (Custom Hotkeys).hotkeys["^g"] (windowGroupID 2 = "📂 文件对话框" ahk_class #32770, actionTypeID 9 → callMap[9] → `QuickSwitchGoto()` 薄壳 → `QuickSwitchRun()`), 仅在文件对话框内生效; 而核心功能本就与热键无关 —— `_QuickSwitchPoll` (800ms) 常驻轮询, 对话框出现即 `_QuickSwitchCollect` → `_QuickSwitchTryAutoJump` (:159-188, 打开型 autoJumpOpen 默认开 / 另存型 autoJumpSave 已完整实现默认关, 守卫齐备: 同实例只跳一次 / collectEnabled 总闸 / DlgClassify 严判 / 同目录不跳 / 排除前缀 / DirExist / DlgJumpTo 三重复核)。**改动为纯减法**: ① 删 data/config.json 的 "^g" 条目 (仓库默认配置与部署树 live 配置两处); ② 重新生成 bin/KeyFlux.ahk (km.Map("^g", …) 随之消失; `QuickSwitchGoto`/`QuickSwitchRun` 入口符号**保留** —— callMap[9] 与 golden_test.go:105 零改动, 供将来托盘等非热键通道复用); ③ 注释对齐 3 文件 (QuickSwitch.ahk :10-11/:364 去 ^g 字样、Suspend 机制保留不动; type9_keyflux.ahk:164 注明无默认热键、入口供非热键通道复用; README 补「当前默认配置不绑定热键」); ④ CONTRACTS 本行。**零默认值变更** (autoJumpSave 维持默认关: 自动改保存目录有覆盖用户预期的打扰风险, 原设计有意裁决; 浮层仍自动显示)、**零 Go 改动**、零三端同步。验证: 部署树 bin/KeyFlux.ahk 重新生成后 grep "^g" 命中 0 + `/Validate` exit 0; go test 仅已记录基线红 TestGoldenKeyFluxAHK (与本改动无关)。**⚠ 需重载引擎后验收**: ① 打开型对话框出现 → 自动跳转 + 浮层 (同实例只跳一次); ② 另存型 → 浮层出现、默认不自动跳; ③ Ctrl+G 在文件对话框内不再触发; ④ 其余热键与托盘暂停/恢复不受影响。 |
