...
3. 🔴 **轮廓格式须为 glyf (TrueType), 不接受 CFF/OTF** —— 已知可用的历代字体
   (Iosevka / MiSans-Bold / 得意黑 TTF) **全是 glyf**; exe 对 CFF 的加载路径**无验证先例**
   (其自定义字体集合若硬编码 `DWRITE_FONT_FACE_TYPE_TRUETYPE`, CFF 会直接加载失败)。
   拿到 `.otf` (CFF) 源**必须先转 glyf**。转换要点 (缺一不可, 均为实测踩坑):
