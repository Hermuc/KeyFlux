...
class EverythingMessages {
  ...
  /** 带占位 {1} 的文案。 */
  static F(key, a) {
    return StrReplace(this.T(key), "{1}", a)
  }

  static _zh(key) {
    switch key {
      case "hint_continue": return "已按下 {1}，继续输入可搜索…"
  ...
  static _en(key) {
    switch key {
      case "hint_continue": return "Triggered — keep typing to search…"
      ...
      case "err_empty": return "empty query"
      case "title": return "Everything"
      case "title_key_space": return "Space"
    }
    return key
  }
}
