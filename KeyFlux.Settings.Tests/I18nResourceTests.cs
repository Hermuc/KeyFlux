...
public sealed class I18nResourceTests
{
    ...
    /// 2026-09 插件系统管理面: 2420/2421/2425 改写为「已导入插件/运行时开发中/运行时说明」,
    /// 新增 13 键 (2427 导入插件 / 2428 插件市场 / 2429 空态标题 / 2430 空态说明 / 2431 已导入
    /// / 2432 导入失败 / 2433 市场目录失败 / 2434 安装 / 2435 已安装 / 2436 删除失败 /
    ...
    /// 方案 C7 (阶段一): 选中动作「添加映射」留桩提示条新增 2517 (提示) / 2518 (去创建专属行为) -> 322。
    ///   2517/2518 键号 ≥2503 (2408–2439 与 2500–2502 已占用); zh/en 双非空。
    /// 方案 C7 (阶段一, 用户创建入口): 新增「匹配类型」页与新建向导 31 键
    ///   (2519 匹配类型 / 2520 文本特征 / 2521 自定义文件后缀 / 2522 新建匹配类型 / 2523 显示名 /
    ///    2524 标识 / 2525 匹配规则 / 2527 添加规则 / 2528 后缀集 / 2529 同时创建专属行为 /
    ...
    ///   2390-2400 / 2403 / 2419-2420 / 2440-2499 / 2516 / 2526 / 2549。
    ///   (2503-2511 已于 2026-09-20 被命令框字体设置占用, 见下方条目。)
    ...
    ///   **新增键自 2588 起**。
    /// 2026-09-20 命令框字体设置 (设置页「选项」新增字体卡片): 新增 9 键
    ///   (2503 命令框字体 / 2504 字体文件 / 2505 选择本地字体文件 / 2506 未选择 /
    ///    2507 恢复默认 / 2508 字重 / 2509 常规 / 2510 中等 / 2511 半粗) -> 402。
    ///   回填了原先的空洞 2503-2511 (该段原标注为未占用); **新增键仍自 2588 起**。
    /// 2026-09-21 字重档位扩展为 5 档 (thin/light 由轮廓**腐蚀**得到, 供粗体源字体提供
    ///   更细选择): 新增 2516 粗体 / 2526 极细 / 2549 细 -> 405。
    ///   🔴 血泪教训 —— thin/light 首版误用了 **2517 / 2518**, 而这两个键**早已被占用**
    ///   (SelectedActionPage 的自定义文本类型留桩提示条文案 / 「创建专属行为」按钮)。
    ///   因 JSON **后定义覆盖前定义**, 那两处 UI 文案被静默改成了「极细」/「细」,
    ///   且键数虚增 (405 个定义但解析后仅 403 个有效键, 重复键被吃掉了)。
    ///   修法: 迁到**经查证的真实空洞** 2526 / 2549。
    ///   📌 **加键前必查**: ① 该键在 i18n.json 里**是否已存在**(grep 全仓引用);
    ///   ② 用「行内正则计数 vs json 解析计数」比对是否相等, 不等即有重复键。
    /// 2026-09-21 字体选择的即时校验提示 (修复"选了没反应"无反馈缺陷): 新增 5 键
    ///   (2588 超 32MB / 2589 轮廓格式不支持 / 2590 字体集合只用首个 face /
    ///    2591 已选定重启生效 / 2592 文件无法读取) -> 410。
    ///   动机: 生成端 InstallCommandFont 的失败**一律静默跳过**(字体是表现层资源,
    ///   不阻断脚本生成), 用户选中 83MB 的 .ttc 或 CFF 的 .otf 后界面毫无反应,
    ///   无从得知原因。现把同一套判据前移到 UI 选择时刻当场给出结论。
    ///   2590 是**唯一带占位符** {0} 的键 (文件名), 见 I18n.T(key, args) 重载。
    /// 2026-09-21 清理: **删除孤儿键 2510 (中等/Medium)** -> 409。
    ///   "medium" 档在字重由 4 档扩为 5 档时已被移除 (改用 thin/light), 该键自此
    ///   **全仓零引用** —— 留着既是死资源, 也会让人误以为"中等"仍是个可选档位。
    ///   (删除前已确认: 引用数 0; 且它在下方白名单里也不存在, 属纯孤儿。)
    private const int ExpectedKeyCount = 409;
