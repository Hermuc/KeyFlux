...
- 🧰 **开机自启 + 一键卸载**：自启用 Windows 计划任务 `\KeyFlux`（登录后延迟 3 秒启动、失败自动重试、路径变更自愈）；附带全中文「卸载软件.bat」，二次确认后结束进程、清理自启与残留
- 🚪 **托盘程序一键唤出**：微信/QQ 等最小化到托盘后，快捷键秒级唤出主窗口，不会重复启动新实例
- 🌈 **细节打磨**：使用指南页文档可阅读编辑、滚动条 Fluent 化、强制浅色主题、窗口标识符写法即时校验等
...
- ⚡ **选中动作（完整）**：
  ...
  - **文本特征**：自动识别 链接 / 路径 / 磁力链接 / B 站视频号（AV 号 / BV 号） / 纯文本
  - **内置 5 类文本行为**：打开网址、打开路径（选中内容为注册表路径时自动定位到注册表编辑器对应项）、打开文件夹、磁力下载（唤起默认 BT 工具）、打开 B 站视频（用系统默认方式打开对应视频）
...
- 📝 **使用指南页文档**：还原 Web UI 版式，正文可直接选中复制，编辑入口在页面底部编辑区，保存即时回显
